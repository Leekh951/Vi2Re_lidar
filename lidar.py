...
class PCD:
    ...
    def ROI_filtering(self):
        ...
        mask_x = np.logical_and(points[0, :] > -3, points[0, :] < 2)
        ...
        points = np.delete(points,np.where(points[2,:]<-1.85),axis=1)
        points = np.delete(points,np.where(points[2,:]>0.0),axis=1)
        
        points = np.delete(points,np.where(points[1,:]>10),axis=1)
        points = np.delete(points,np.where(points[1,:]<-10),axis=1)
        
        points = np.delete(points,np.where(points[0,:]>30),axis=1)
        points = np.delete(points,np.where(points[0,:]<-15),axis=1)
    ...
    def o3d_DBscan(self):
        ...
        if (self.pcd is not None):
            self.labels = self.pcd.cluster_dbscan(eps=0.8, min_points=5)
        ...
        for i in range(len(pcd_center)):
            ...
            if (
                0.0 <= bbox_size[0] <= 6.5
                and 0.0 <= bbox_size[1] <= 6.5
                and 0.0 <= bbox_size[2] <= 6.5
                and 1/4 <= bbox_size[0]/bbox_size[1] <= 3
                and bbox_size[1]/bbox_size[0] <= 4
                and 0 <= bbox_size[2]/bbox_size[1] <= 6
            ):
